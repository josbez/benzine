...
|---|---|---|
| E1-1 Pages ontstoppen | ⛔ **oorzaak niet vastgesteld** | Wat vaststaat: beide routes eindigen in dezelfde hangende `deployment_in_progress` (run `31110887125` — de push naar `gh-pages` triggert GitHubs eigen *pages build and deployment*, die intern `deploy-pages` draait). Wat is uitgesloten: de Pages-bron staat geverifieerd goed (*Deploy from a branch*, `gh-pages` / `(root)`), en billing is in orde (GitHub Free, geen limiet bereikt). Wat er bijkwam: die avond liep er een GitHub Actions-storing waarvan de statusmeldingen Pages expliciet noemen, met jobs die wel in de wachtrij kwamen maar nooit een runner kregen — of die ook de eerdere stalls dekt hangt af van het starttijdstip van de storing, en dat is niet vastgesteld. Volgorde staat in de README; stap 1 is simpelweg opnieuw draaien zodra de storing voorbij is. |
...
| E2-4 GLA-scraper live valideren | 🔶 **gediagnosticeerd, nog niet gefixt** | De eerste echte run (7 aug, `31152665906`) leverde het antwoord: de pagina komt binnen (8011 tekens, juiste titel, geen 403 of cookiewall) maar bevat nul prijs-tokens en nul brandstoflabels — de cijfers komen na render. Automatisch gemeld in issue #17. Omdat deze sessie de pagina zelf niet kan bereiken, draait de diagnose nu op de runner: `make probe` kijkt ín de `<script>`-tags die de tekstparser juist wegstript (server-rendered frameworks zetten hun data als JSON in de HTML, dus "client-side gerenderd" en "niet in de HTML" zijn niet hetzelfde) en volgt de API-achtige URL's die de pagina noemt. De uitkomst wordt aan de issue gehangen. De volgende gefaalde run vertelt dus waar de prijs staat. |
